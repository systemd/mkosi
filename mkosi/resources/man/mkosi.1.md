...
:   Takes one of `none`, `systemd-boot`, `uki`, `uki-prebuilt`, `grub`,
    ...
    If set to `uki-prebuilt`, **mkosi** copies a unified kernel image already
    ...
    directory that is identified as a UKI is used.
...
1. The package cache of the distribution package manager is cached
   between builds. The location can beconfigured with the
   `PackageCacheDirectory=` option or the `mkosi.pkgcache/` directory.
   This form of caching relies on the distribution's package manager,
...
1. Finally, between multiple builds the build artifact directory may
   ...
   process of a `mkosi.build` build script.
