...
class Installer(DistributionInstaller):
    ...
    def repository_variants(
        ...
        if mirror == "https://composes.stream.centos.org" and not context.config.snapshot:
            die(
                f"Snapshot= must be used on {cls.pretty_name()} if Mirror=https://composes.stream.centos.org"
            )
        ...
        if context.config.local_mirror:
            ...
            if mirror == "https://composes.stream.centos.org":
                if GenericVersion(context.config.release) == 9:
                    ...
                    subdir = f"stream-{context.config.release}"

                subdir = f"{subdir}/production"
            elif mirror == "https://mirror.facebook.net" and context.config.snapshot:
                subdir = f"centos-composes/{context.config.release}"
            elif repo == "extras":
                subdir = "SIGs/$stream"
            else:
                subdir = "$stream"
            ...
            if context.config.snapshot:
                ...
                try:
                    curl(context.config, join_mirror(mirror, subdir))
                except subprocess.CalledProcessError:
                    die(
                        f"Could not access snapshot at {join_mirror(mirror, subdir)}",
                        hint="Does the snapshot still exist?",
                    )
    ...
    def latest_snapshot(cls, config: Config) -> str:
        ...
        if mirror == "https://mirror.facebook.net":
            subdir = f"centos-composes/{config.release}"
        else:
            if GenericVersion(config.release) == 9:
                subdir = ""
            else:
                subdir = f"stream-{config.release}"
