...
from mkosi.log import log_info, log_warning
...
def parse_fedora_release(release: str) -> tuple[str, str]:
    if release.startswith("rawhide-"):
        ...
        log_info(f"Fedora rawhide — release version: {releasever}")
    ...
    with state.workspace.joinpath("dnf.conf").open("w") as f:
        ...
        for repo in repos:
            ...
            if repo.gpgpath.exists():
                ...
                log_warning(f"GPG key not found at {repo.gpgpath}. Not checking GPG signatures.")
                gpgcheck = False
