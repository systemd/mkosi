...
def save_manifest_to_image(context: Context, manifest: Optional[Manifest]) -> None:
    ...
    if ManifestFormat.json in context.config.manifest_format and manifest.has_data():
        if context.config.output_format.is_extension_image():
            manifest_path = "usr/lib/mkosi/extension-manifest.d/.manifest"
        else:
            manifest_path = "usr/lib/mkosi/.manifest"
        osmanifest = context.root / manifest_path
        osmanifest.parent.mkdir(parents=True, exist_ok=True)
