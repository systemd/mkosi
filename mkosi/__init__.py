...
def luks_format(dev: str, passphrase: Dict[str, str]) -> None:
    if passphrase['type'] == 'stdin':
        ...
        run(["cryptsetup", "luksFormat", "--force-password", "--pbkdf-memory=64", "--pbkdf-parallel=1", "--pbkdf-force-iterations=1000", "--batch-mode", dev], input=passphrase_content, check=True)
    else:
        assert passphrase['type'] == 'file'
        run(["cryptsetup", "luksFormat", "--force-password", "--pbkdf-memory=64", "--pbkdf-parallel=1", "--pbkdf-force-iterations=1000", "--batch-mode", dev, passphrase['content']], check=True)
...
def mount_image(args: CommandLineArguments,
    ...
    try:
        ...
        with complete_step('Unmounting image'):
            ...
            # command.
            if args.output_format == OutputFormat.gpt_btrfs:
                run(["btrfs", "device", "scan", "-u"], check=True)
...
def install_opensuse(args: CommandLineArguments, root: str, do_run_build_script: bool) -> None:
    ...
    # "zypper install".
    #
    run(["zypper", "--root", root, "addrepo", "-ck", release_url, "repo-oss"], check=True)
    run(["zypper", "--root", root, "addrepo", "-ck", updates_url, "repo-update"], check=True)
    ...
    if release.startswith("42."):
        packages += ["patterns-openSUSE-minimal_base"]
    else:
    ...
    #
    run(["zypper", "--root", root, "modifyrepo", "-K", "repo-oss"], check=True)
    run(["zypper", "--root", root, "modifyrepo", "-K", "repo-update"], check=True)
...
def create_parser() -> ArgumentParserMkosi:
    ...
    group.add_argument("--var-size",
                       help='Set size of /var partition (only gpt_ext4, gpt_xfs, gpt_squashfs)', metavar='BYTES')
    group.add_argument("--tmp-size",
                       help='Set size of /var/tmp partition (only gpt_ext4, gpt_xfs, gpt_squashfs)', metavar='BYTES')
...
def print_summary(args: CommandLineArguments) -> None:
    ...
    if args.output_format.is_disk():
        ...
        sys.stderr.write("        /var Partition: " + format_bytes_or_disabled(args.var_size) + "\n")
        sys.stderr.write("    /var/tmp Partition: " + format_bytes_or_disabled(args.tmp_size) + "\n")
...
def build_image(args: CommandLineArguments,
    ...
    with attach_image_loopback(args, raw) as loopdev:
        ...
        with luks_setup_all(args, loopdev, do_run_build_script) as (encrypted_root, encrypted_home, encrypted_srv, encrypted_var, encrypted_tmp):
