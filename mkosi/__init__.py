...
def check_inputs(config: Config) -> None:
    ...
    if config.verity == Verity.signature and not config.verity_key:
        die(
            "Verity= is enabled but no verity key is configured",
            hint="Run mkosi genkey to generate a key/certificate pair",
        )

    if config.verity == Verity.signature and not config.verity_certificate:
        die(
...
def make_image(
    ...
    if context.config.verity == Verity.signature and not any(
...
def want_verity(config: Config) -> bool:
    return config.verity == Verity.signature or bool(
...
def make_extension_or_portable_image(context: Context, output: Path) -> None:
    if want_verity(context.config) or context.config.verity == Verity.signature:
        unsigned = ""
    else:
        unsigned = "-unsigned"
    ...
    if ArtifactOutput.partitions in context.config.split_artifacts:
        cmdline += ["--split=yes"]
    if context.config.verity == Verity.verity:
...
def validate_certificates_and_keys(config: Config) -> None:
    ...
    if config.verity != Verity.off and config.verity_certificate and config.verity_key:
        run_systemd_sign_tool(
            config,
